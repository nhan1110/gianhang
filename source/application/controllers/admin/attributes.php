...
class Attributes extends MY_Controller {
    ...
    public function index() {
        if ($this->input->get("category-type")) {
            ...
            if (count($check_cat_type) > 0) {
                ...
                $all_attr = $attribute->get_attribute_by_cat($data_id_cat_type,0,$this->type_member);
                $sort_get_gr = "";
                $sort_get_attr_parent = "";
                $element = "";
                $type_parent = array("select", "multipleselect", "multipleradio");
                $tree_attr = "<ul id = 'tree_attr' class='list-group droptrue level-0 tree_parent' data-level = '0'>";
                foreach ($all_grp as $key => $value) {
                    ...
                    $tree_attr.="<div class='list-group-item'><span id='name-attribute'>" . $value["Name"] . "</span> (Category type " . $value["Ct_Name"] . ").";
                        ...
                        if($data_id_cat_type == $value["Category_Type_ID"]){
                            ...
                            $tree_attr.='<a href="#" id="action-edit" data-id ="' . $value["ID"] . '" data-type ="attribute-group"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                            <a href="#" id="action-delete" data-id ="' . $value["ID"] . '" data-type ="attribute-group"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>';
                        }
                    ...
                    foreach ($arg_gr_attr as $key_1 => $value_1) {
                        if ($value_1["Group_ID"] == $value["ID"]) {
                            ...
                            $tree_attr.="<div class='list-group-item'><span id='name-attribute'>" . $value_1["Name"] . "</span>";          
                                $tree_attr.='<div class="actions">';
                                if($data_id_cat_type == $value_1["Category_Type_ID"]){
                                    if ($this->type_member == "System") {
                                    ...
                                    $tree_attr.='<a href="#" id="action-edit" data-id ="' . $value_1["ID"] . '" data-type ="attribute-group-attribute"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                                    <a href="#" id="action-delete" data-id ="' . $value_1["ID"] . '" data-type ="attribute-group-attribute"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>';
                                }
                            ...
                            if (in_array($value_1["Value"], $type_parent)) {
                                ...
                                foreach ($all_attr as $key_2 => $value_2) {
                                    if ($value_2["Parent_ID"] == $value_1["ID"]) {
                                        ...
                                        if($data_id_cat_type == $value_2["Category_Type_ID"]){
                                            $tree_attr.='<div class="actions">';
                                            if ($this->type_member == "System") {
                                                $tree_attr.="<select data-type ='attribute' id='change-type' data-id ='" . $value_1["ID"] . "'>";
                                                if ($value_1["Type"] == "System") {
                                            ...
                                            $tree_attr.='<a href="#" id="action-edit" data-id ="' . $value_2["ID"] . '" data-type ="attribute"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> 
                                                <a href="#" id="action-delete" data-id ="' . $value_2["ID"] . '" data-type ="attribute"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>     
                                            </div>';
                                        }
                                        $tree_attr.="</div>";
                                        $tree_attr.="</li>";
                ...
                $tree_attr .= "</ul><input type='hidden' id='root-type' value ='". $check_cat_type["Slug"]."'>";
            ...
            $table = $category_Type->order_by("Sort","ASC")->get_raw()->result_array();
            $this->load->library('Helperclass');
    ...
    public function add() {
        ...
        if (count($check_cat_type) > 0 && $name != "" && in_array($type, $arg_type)) {
            ...
            $messenger_error = $this->input->post("messenger_error");
            $show_search = $this->input->post("show_search");
            $show_search = ($show_search == "1" || $show_search == "0") ? $show_search  : "0";
            $attribute = new Attribute();
            $path = "/" . $slug . "/";
            ...
            $attribute->Messenger_Error = @$messenger_error;
            $attribute->Show_Search = @$show_search;
            ...
            $type_parent = array("select", "multipleselect", "multipleradio");
            if (trim($default_values) != "") {
            	$arg_default_values = explode("[*_*]", trim($default_values));
            } else if (trim($multivalue) != "") {
    ...
    public function edit(){
        ...
        $arg_type = ["attribute-group","attribute-group-attribute","attribute"];
        $table = "";
        if(in_array($type,$arg_type)){
            if($type == "attribute-group"){
                $table = new  Attribute_Group();
            }else{
                $table = new  Attribute();
            }
            $record = $table->where("ID",$id)->get_raw()->row_array();
    ...
    public function add_group() {
        ...
        if ($data_name != "" && count($check_cat_type) > 0) {
            ...
            $max_sort = $attribute_group->get_sort($category_type_id,0,$this->type_member);
            if ($max_sort == null) {
                $max_sort = 0;
            } else {
                $max_sort = $max_sort["Sort"] + 1;
        ...
        if ($type != null && $id != null && is_numeric($id) && in_array($type, $type_parent)) {
            ...
            switch ($type) {
                ...
                case "attribute-group-attribute":
                    ...
                    if ($check != null) {
                        $this->db->delete("Attribute", array('ID' => $check["Attribute_ID"]));
                        $this->db->delete("Attribute", array('Parent_ID' => $check["Attribute_ID"]));
                    }
                    break;
                default:
                    $masterclass = "Attribute";
                    $attribute = new Attribute();
                    $check = $attribute->where("ID", $id)->get_raw()->row_array();
                    $this->db->delete("Attribute", array('Parent_ID' => $id));
                    break;
            }
    ...
    public function update_attribute(){
        ...
        $show_search = $this->input->post("show_search");
        $attribute = new Attribute();
        $record = $attribute->where("ID",$id)->get_raw()->row_array();
        $path = "";
        if(!empty($record)){
            if($name != $record["Name"]){
                $this->load->library('Helperclass');
                $slug = $this->helperclass->slug("Attribute", "slug", $name);
                $path = "/" . $slug . "/";
                $attribute->replace_slug("Attribute", $record["Path"], $path, $record["ID"]);
                $data_update["Name"] = $name;
                $data_update["Slug"] = $slug;
                $data_update["Path"] = $path;
            }
            $data_update["Require"] = $required;
            $data_update["Validate"] = $validate;
            $data_update["Unit"] = $unit;
            $data_update["Value"] = $type;
            $data_update["Messenger_Error"] = $messenger_error;
            $data_update["Show_Search"] = $show_search;
            $attribute->where("ID", $id)->update($data_update);
            $record = $attribute->where("ID",$id)->get_raw()->row_array();
            $attribute->like("Path",$record["Path"])->update(["Show_Search" => $show_search]);
            $data["success"] = "success";
            $data["id"] = $id;
        }
        die(json_encode($data));
