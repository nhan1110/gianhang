...
class Members_group extends MY_Controller {
    ...
    public function index() 
    {
        ...
        if (isset($_POST) && $this->input->post('text_title') !== FALSE) {
            if ($this->form_validation->run() === FALSE) {
                ...
                $bol = false;
                $role = new Member_Group();
                $id = $this->input->post('id');
                if (is_numeric($id) && floatval($id) > 0) {
                    $arr = array(
                        'Group_Title' => $this->input->post('text_title'),
                        'Group_Description' => $this->input->post('text_description')
                    );
                    $bol = $role->where('ID', $id)->update($arr);
                } else {
                    $role->Group_Title = $this->input->post('text_title');
                    $role->Group_Description = $this->input->post('text_description');
                    $bol = $role->save();
                }
                // Save new user
                if ($bol) {
        ...
        $pagination_config = array(
            'base_url' => site_url('admin/members_group'),
        ...
        $this->load->view('backend/members/member_group', $data);
    ...
    function delete($id = null) 
    {
        ...
        redirect(site_url('admin/members_group/'));
